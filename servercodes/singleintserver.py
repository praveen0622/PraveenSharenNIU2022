...
def recv_from_client(client_socket,addr):
	global command
	command='50'
	t2 = threading.Thread(target=video_to_client,args=(client_socket,addr,command))
	t2.start()
...
t1 = threading.Thread(target=recv_from_client,args=(client_socket,addr))
t1.start()
t1.join()
